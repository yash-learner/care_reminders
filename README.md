...
Plugin URLs mount at `/api/care_reminders/` (health: `/api/care_reminders/health`). OTP session: `POST /sync/` (does not arm), `GET /alarms/`, `GET /doses/` (taken / skipped / missed / upcoming), `GET`/`PATCH /clocks/`, `POST /arm/` / `POST /disarm/`, and signed take / skip / snooze / fired.
